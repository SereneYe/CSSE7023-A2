...
"src/sheep/parsing/*", # I don't know if this works :/
"src/sheep/ui/graphical/javafx/SheepApplication.java",
"src/sheep/ui/graphical/javafx/*", # or this
...
$local:destination_path = "submission.zip" 

# Backup existing zip to this directory
$local:backup_path = "submission_history" 
...
$local:paths | ForEach-Object {
    if (Test-Path $_) {
		...
		Copy-Item -Recurse -Container $_ $local:temp_path/$local:parent
    } else { 
